...
def build_network(net_name, ae_net=None):
    ...
    implemented_networks = ('mnist_LeNet', 'mnist_DGM_M2', 'mnist_DGM_M1M2',
                            ...
                            'shuttle_DGM_M2', 'thyroid_DGM_M2','PM2_5_mlp','PM2_5_mnist_LeNet','description_mlp','heatmap_mlp','description_plus_timeseries_mlp','line_chart_all_mlp')
    assert net_name in implemented_networks

    net = None
    
   
    if net_name == 'description_mlp':
       net = MLP(x_dim=30, h_dims=[256, 128], rep_dim=8, bias=False)
       
    if net_name == 'heatmap_mlp':
       net = MLP(x_dim=784, h_dims=[256, 128], rep_dim=8, bias=False)
       
    if net_name == 'description_plus_timeseries_mlp':
       net = MLP(x_dim=90, h_dims=[256, 128], rep_dim=8, bias=False)
       
    if net_name == 'line_chart_all_mlp':
       net = MLP(x_dim=784, h_dims=[256, 128], rep_dim=8, bias=False)
    ...
    if net_name == 'PM2_5_mlp':
        net = MLP(x_dim=784, h_dims=[256, 128], rep_dim=8, bias=False)
        #net = DeepGenerativeModel([1*28*28, 2, 32, [128, 64]])
    
    if net_name == 'newdata_mlp':
        net = MLP(x_dim=30, h_dims=[128, 64], rep_dim=8, bias=False)
        #net = DeepGenerativeModel([1*28*28, 2, 32, [128, 64]])
...
def build_autoencoder(net_name):
    ...
    implemented_networks = ('mnist_LeNet', 'mnist_DGM_M1M2','fmnist_LeNet', 'fmnist_DGM_M1M2','cifar10_LeNet', 'cifar10_DGM_M1M2',                
                            'arrhythmia_mlp', 'cardio_mlp', 'satellite_mlp', 'satimage-2_mlp', 'shuttle_mlp',
    ...
    if net_name == 'heatmap_mlp':
        ae_net = MLP_Autoencoder(x_dim=784, h_dims=[32, 16], rep_dim=8, bias=False)
       
    if net_name == 'description_plus_timeseries_mlp':
        ae_net = MLP_Autoencoder(x_dim=90, h_dims=[32, 16], rep_dim=8, bias=False)
